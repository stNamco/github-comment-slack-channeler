...
async function createBlocks(githubToken, config, url, unfurlLinksContextStr) {
  ...
  async function fetchComment(githubToken, owner, repoName, type, typeId) {
  ...
  async function unfurlIfNeeded(unfurlLinksContextStr, githubToken, body) {
    if (unfurlLinksContextStr === 'false') {
      return []
    }

    let urls = body.match(/https:\/\/github.com[-_.!~*\'()a-zA-Z0-9;\/?:\@&=+\$,%#]+(discussion_r\d{9}|issuecomment-\d{9}|files#r\d{9})/g);
    if (urls === null) {
      return []
    }
  ...
  let body = comment.body.startsWith(targetConfig.key) ? comment.body.slice(targetConfig.key.length) : comment.body;
  let blocks = [
      {
        "type": "section",
        "text": {
  ...
  blocks = blocks.concat(await unfurlIfNeeded(unfurlLinksContextStr, githubToken, body));
...
(async () => {
  ...
  try {
    ...
    const [channelId, blocks] = await createBlocks(githubToken, config, githubCommentUrl, unfurlLinksContextStr).catch(e => console.error(e));
